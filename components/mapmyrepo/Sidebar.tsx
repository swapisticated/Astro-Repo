...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"DETAILS" | "CHAT" | "NODES">(
    "DETAILS"
  );
  ...
  const handleSendMessage = async () => {
    ...
    if (node) {
      ...
      answer = await askUniverseQuestion(
        universeNode,
        universeData,
        userMsg.text
      );
  ...
  const widthClass =
    isCollapsed || !activeNode
      ...
      : "w-[calc(100vw-1rem)] md:w-[400px] translate-x-0";

  return (
    <div
      className={`${panelClasses} ${widthClass} ${
        isPencil || isComic ? "font-['Patrick_Hand']" : ""
      }`}
    >
      {/* Collapsed Dock Content */}
      <div
        className={`flex flex-col items-center py-6 h-full transition-opacity duration-300 ${
          !isCollapsed && activeNode ? "hidden" : "flex"
        }`}
      >
        <button
          ...
          <>
            ...
            <div
              className={`p-6 pl-16 border-b ${
                isComic
                  ? "border-black bg-[#f0e6d2]"
                  : "border-black/5 bg-white/40"
              }`}
            >
              <div className="flex items-start gap-4">
                <div
                  className={`p-2.5 rounded-xl border text-2xl shadow-sm backdrop-blur-md 
                                    ${
                                      isComic
                                        ? "bg-[#fff] border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                  ...
                  <div className="flex items-center gap-2">
                    <h2
                      className={`text-lg font-bold truncate tracking-tight ${
                        isComic ? "text-black" : "text-[#1f2328]"
                      }`}
                    >
                      {activeNode.name}
                    </h2>
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    <span
                      className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wider border 
                                            ${
                                              isComic
                                                ? "bg-[#fff] text-black border-black"
                    ...
                    {node && (
                      <div
                        className={`text-xs font-mono truncate opacity-60 ${
                          isComic ? "text-black" : "text-[#656d76]"
                        }`}
                        title={node.path}
                      >
                        {node.path}
                    ...
                    {universeNode && universeNode.type === "REPO" && (
                      <div
                        className={`text-xs font-mono truncate opacity-60 ${
                          isComic ? "text-black" : "text-[#656d76]"
                        }`}
                      >
                        ⭐ {universeNode.stargazers_count}
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Navigation */}
              <div
                className={`flex gap-1 mt-6 p-1.5 rounded-xl ${
                  isComic ? "bg-black/10" : "bg-black/5"
                }`}
              >
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all duration-300 ${
                    activeTab === "DETAILS"
                      ? isComic
                        ? "bg-[#fff] text-black border border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                        : "bg-white text-[#1f2328] shadow-lg shadow-black/5"
                      : isComic
                ...
                <div className="h-full overflow-y-auto p-6 space-y-8 custom-scrollbar">
                  ...
                  {node && node.summary ? (
                    <div className="animate-fadeIn">
                      <h3
                        className={`text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2 opacity-70 ${
                          isComic ? "text-black" : "text-[#1f2328]"
                        }`}
                      >
                      ...
                      >
                        <div
                          dangerouslySetInnerHTML={renderMarkdown(node.summary)}
                        />
                      </div>
                    </div>
                  ) : (
                          ...
                          >
                            <div
                              className={`w-5 h-5 border-2 border-t-transparent rounded-full animate-spin ${
                                isComic ? "border-black" : "border-[#656d76]"
                              }`}
                            ></div>
                    ...
                    <div className="animate-fadeIn">
                      {universeNode.type === "REPO" && universeNode.data && (
                        <div className="space-y-4">
                          <div>
                            <h3
                              className={`text-xs font-bold uppercase tracking-wider mb-2 opacity-70 ${
                                isComic ? "text-black" : "text-[#1f2328]"
                              }`}
                            >
                              Description
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            ...
                            >
                              ...
                              <div className="text-lg font-bold">
                                {universeNode.data.stargazers_count}
                              </div>
                            </div>
                            <div
                              className={`p-3 rounded-lg border ${
                            ...
                            >
                              ...
                              <div className="text-lg font-bold">
                                {universeNode.data.language || "N/A"}
                              </div>
                            </div>
                            <div
                              className={`p-3 rounded-lg border ${
                            ...
                            >
                              ...
                              <div className="text-lg font-bold">
                                {universeNode.data.open_issues_count}
                              </div>
                            </div>
                          </div>
                          <button
                            className={`w-full py-2 rounded-lg font-bold text-sm transition-all ${
                              isComic
                                ? "bg-[#2da44e] text-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-[#2c974b]"
                                : "bg-[#2da44e] text-white hover:bg-[#2c974b]"
                            }`}
                            onClick={() =>
                              window.open(universeNode.data?.html_url, "_blank")
                            }
                          >
                            View on GitHub
                          </button>
                          ...
                          <div className="pt-4">
                            ...
                            <div
                              className={`border rounded-xl divide-y overflow-hidden ${
                                isComic
                                  ? "border-black divide-black bg-white"
                                  : "border-black/5 divide-black/5"
                              }`}
                            >
                              {universeData
                                .filter(
                                  ...
                                  >
                                    ...
                                    <span
                                      className={`ml-auto text-[10px] px-2 py-0.5 rounded-full font-medium ${
                                        isComic
                                          ? "text-black bg-[#f0e6d2] border border-black"
                                          : "text-[#656d76] bg-[#eff1f3]"
                                      }`}
                                    >
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="pt-4">
                            ...
                            >
                              {[
                                ...new Set(
                                  universeData.map((r) => r.language || "Other")
                                ),
                              ]
                                .sort()
                                .map((lang, idx) => (
                  ...
                  <hr
                    className={`border-t ${
                      isComic ? "border-black" : "border-black/5"
                    }`}
                  />
                      ...
                      <div
                        className={`rounded-xl border overflow-hidden shadow-inner ${
                          isComic
                            ...
                            : "border-black/5 bg-[#f6f8fa]/50"
                        }`}
                      >
                        <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-4">
                          <pre
                            className={`text-xs font-mono whitespace-pre-wrap break-words leading-relaxed bg-transparent p-0 border-none m-0 ${
                              isComic ? "text-black" : "text-[#1f2328]"
                            }`}
                          >
                    ...
                    <div className="animate-fadeIn delay-200">
                      ...
                      >
                        {node.children.map((child, idx) => (
                          <div
                            key={idx}
                            className={`flex items-center gap-3 p-3 px-4 transition-colors ${
                              isComic
                ...
                <div className="h-full flex flex-col">
                  ...
                  >
                    ...
                    {chatHistory.map((msg, i) => (
                      ...
                      >
                        <div
                          className={`max-w-[90%] rounded-2xl p-4 text-sm leading-relaxed border shadow-sm backdrop-blur-md ${
                            msg.role === "user"
                              ? isComic
                                ? "bg-[#89CFF0] text-black border-black border-2 rounded-br-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                                : "bg-[#0969da]/10 text-[#1f2328] border-[#0969da]/20 rounded-br-sm"
                              : isComic
                              ? "bg-white text-black border-black border-2 rounded-bl-sm shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]"
                              : "bg-white/80 text-[#1f2328] border-white/40 rounded-bl-sm"
                          }`}
                        >
                          {msg.role === "user" ? (
                    ...
                    {isTyping && (
                      <div className="flex justify-start">
                        <div
                          className={`rounded-2xl rounded-bl-sm p-4 border flex gap-1.5 shadow-sm ${
                            isComic
                  ...
                  >
                    <div className="relative flex gap-2">
                      <input
                        type="text"
                        className={`flex-1 border rounded-xl py-2.5 px-4 text-sm focus:outline-none focus:ring-2 focus:border-transparent bg-transparent transition-all ${
                          isComic
                      ...
                      <button
                        ...
                        className={`px-4 rounded-xl disabled:opacity-50 transition-all font-semibold text-sm shadow-lg hover:shadow-xl active:scale-95 ${
                          isComic
                            ? "bg-[#0969da] text-white border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-[#0860ca]"
