...
import {
  parseFilesToTree,
  parseFileEntryToTree,
  fetchGithubRepo,
} from "@/services/fileService";
import { FileSystemNode } from "@/types/mapmyrepo";
...
export const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const handleLoadFromGitHub = async () => {
    ...
    try {
      ...
      const tree = await fetchGithubRepo(
        repoUrl.trim(),
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center h-full w-full transition-colors duration-300 relative overflow-hidden
      ...
      <div className="z-10 max-w-2xl w-full px-6">
        ...
        <div className="mb-8 text-center">
          <p
            className={`text-sm text-black ${
              isPencil || isComic ? "text-lg font-bold tracking-wider" : ""
            }`}
          >
          ...
          <input
            ...
            className={`flex-1 border py-2 px-3 text-sm focus:outline-none transition-all 
                        ${
                          isPencil || isComic
                            ? isComic
                              ? "bg-white border-black text-black placeholder-black/50 focus:border-black focus:ring-1 focus:ring-black"
                              : "bg-white border-black text-black placeholder-gray-500 focus:border-black focus:ring-1 focus:ring-black"
          ...
          <button
            ...
            className={`px-4 py-2 text-white disabled:opacity-50 text-sm transition-all transform active:scale-95
                        ${
                          isPencil || isComic
                            ? isComic
                              ? "bg-[#ffcc00] text-black border-2 border-black hover:bg-[#ffdb4d] font-bold disabled:text-black/50"
                              : "bg-black text-white border-2 border-transparent hover:border-black hover:bg-white hover:text-black font-bold"
        ...
        <div className="relative my-6">
          <div className="absolute inset-0 flex items-center">
            <div
              className={`w-full border-t ${
                isPencil || isComic ? "border-black/30" : "border-[#d0d7de]"
              }`}
            ></div>
          </div>
          <div className="relative flex justify-center text-sm">
            <span
              className={`px-2 ${
                isPencil || isComic ? "font-bold" : ""
              } bg-[#ffffff] text-[#656d76]`}
            >
              OR
            </span>
          </div>
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center gap-3">
              <div
                className={`w-5 h-5 border-2 border-t-transparent rounded-full animate-spin border-[#656d76]`}
              ></div>
            ...
            <>
              <div
                className={`mb-2 transition-colors text-[#656d76] group-hover:text-[#1f2328]`}
              >
                <svg
                ...
                >
                  <path
                    ...
                    strokeWidth={isPencil || isComic ? 2.5 : 1.5}
                    d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z"
                  />
                </svg>
                <span
                  className={`text-sm block text-center ${
                    isPencil || isComic ? "font-bold text-lg" : "font-semibold"
                  }`}
                >
                  Upload a directory
                </span>
        ...
        <div
          className={`mt-6 flex items-center justify-center gap-2 text-xs border p-3 shadow-sm 
                    ${
                      isPencil || isComic
        ...
        >
          <svg
            className={`w-4 h-4 flex-shrink-0 text-[#656d76]`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <p>
            <span className="font-semibold">Pro tip:</span> Dragging a folder
            directly onto the box above is the fastest way.
          </p>
        </div>
      </div>
