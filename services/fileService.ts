import { FileSystemNode, NodeType } from "@/types/mapmyrepo";

export const parseFilesToTree = async (
  fileList: FileList
  ...
  const sortedFiles = Array.from(fileList).sort(
    (a, b) => a.webkitRelativePath.length - b.webkitRelativePath.length
  );
...
export const parseFileEntryToTree = async (
  entry: any
): Promise<FileSystemNode> => {
          ...
          async (file: File) => {
            ...
            if (file.size < 200000) {
              try {
                ...
                console.warn("Read error", ent.name);
              }
            }
            resolve();
          },
...
const calculateSizeRecursive = (node: FileSystemNode): number => {
  ...
  if (node.children) {
    const sum = node.children.reduce(
      (acc, child) => acc + calculateSizeRecursive(child),
      0
    );
    node.value = sum;
    return sum;
...
export const fetchGithubRepo = async (
  ...
  if (repoResponse.status === 403) {
    throw new Error(
      "GitHub API rate limit exceeded. Please provide a GitHub Token (PAT) to increase limits."
    );
  }
  ...
  const calculateSizeRecursive = (node: FileSystemNode): number => {
    ...
    if (node.children) {
      const sum = node.children.reduce(
        (acc, c) => acc + calculateSizeRecursive(c),
        0
      );
      node.value = sum;
      return sum;
  ...
  const sortedItems = treeData.tree.sort(
